...
class ParseMultiRobotPose():
    ...
    def __init__(self, target_argument: Text):
        ...
        for example, `ros2 launch nav2_bringup bringup_multirobot_launch.py
        robots:="robot1={x: 1.0, y: 1.0, yaw: 0.0};              robot2={x:
        1.0, y: 1.0, z: 1.0, roll: 0.0, pitch: 1.5707, yaw: 1.5707}"`

        `target_argument` shall be 'robots'. Then, this will parse a string
        value for `robots` argument.

        Each robot name which is corresponding to namespace and pose of it will
        be separted by `;`. The pose consists of x, y and yaw with YAML format.
