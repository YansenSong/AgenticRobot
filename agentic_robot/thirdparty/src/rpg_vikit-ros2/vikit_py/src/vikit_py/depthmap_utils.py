...
def load_depthmap(depthmap_full_file_path, depthmap_rows, depthmap_cols,
    ...
    if depthmap_full_file_path.endswith('.bin'):
        try:
            # the separator character '' specifies a binary file
            depth_array = np.fromfile(
                depthmap_full_file_path, fileformat, -1, '')
        except IOError:
            print(
                'Could not open file ' +
                depthmap_full_file_path +
        ...
        if depthmap_full_file_path.endswith('.depth'):
            try:
                if is_megapov_depthmap:
                    depth_array = np.fromfile(
                        depthmap_full_file_path, dtype='>d')
                else:
                    # the separator character ' ' specifies a text file
                    depth_array = np.fromfile(
                        depthmap_full_file_path, np.float32, -1, ' ')
            except IOError:
                print(
                    'Could not open file ' +
                    depthmap_full_file_path +
