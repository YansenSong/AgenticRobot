...
def align_se3(model, data, precision=False):
    ...
    if (np.linalg.det(U) * np.linalg.det(Vh) < 0):
        S[2, 2] = -1
    R = U * S * Vh
    t = data.mean(1).reshape(data.shape[0], 1) -\
        R * model.mean(1).reshape(model.shape[0], 1)
...
def hand_eye_calib(q_gt, q_es, p_gt, p_es, I, delta=10, verbose=True):
    ...
    for ix, i in enumerate(I):
        ...
        b_A[3 * ix:3 * ix + 3,
            0] = np.dot(np.transpose(A1), p_es[i + delta, :] - p_es[i, :])
        b_B[3 * ix:3 * ix + 3,
            0] = np.dot(np.transpose(B1), p_gt[i + delta, :] - p_gt[i, :])

    # compute rotation
    D, V = np.linalg.linalg.eig(np.dot(M.transpose(), M))
    Lambda = np.diag(
        [np.sqrt(1.0 / D[0]), np.sqrt(1.0 / D[1]), np.sqrt(1.0 / D[2])])
    ...
    d = np.zeros([3 * n, 1])
    for i in range(n):
        d[3 * i:3 * i + 3, :] = b_A[3 * i:3 * i + 3, :] -\
            np.dot(X, b_B[3 * i:3 * i + 3, :])

    b = np.dot(
        np.linalg.inv(
            np.dot(
                np.transpose(C), C)), np.dot(
            np.transpose(C), d))
