...
def pcd2keyframe(
    ...
    for kf_id in tqdm(
        ...
        if intensities is not None:
            ...
            temp_colors[:, 0] = intensity_values / np.max(intensity_values) if np.max(
                intensity_values) > 0 else intensity_values  # 存储归一化的intensity在R通道
        ...
        if intensities is not None and len(downsampled_pcd.colors) > 0:
            ...
            # 从R通道恢复原始intensity值
            downsampled_intensities = recovered_colors[:, 0] * np.max(
                intensities[idx]) if np.max(intensities[idx]) > 0 else recovered_colors[:, 0]

        # 创建新的张量点云对象，只保留xyz和intensity属性
        sub_pcd_t = o3d.t.geometry.PointCloud(
            o3d.core.Tensor(downsampled_points))

        # 添加降采样后的intensity信息
        if downsampled_intensities is not None:
            sub_pcd_t.point.intensity = o3d.core.Tensor(
                downsampled_intensities.reshape(-1, 1).astype(np.float32))
