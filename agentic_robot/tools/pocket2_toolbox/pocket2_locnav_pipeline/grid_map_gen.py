...
def statistical_outlier_removal(pcd, nb_neighbors=30, std_ratio=1.0):
    print(
        f'[grid_map_gen] 统计离群点滤波: nb_neighbors={nb_neighbors}, std_ratio={std_ratio}')
...
def pass_through_filter(pcd, z_min, z_max, negative=False):
    print(
        f'[grid_map_gen] 直通滤波: z_min={z_min}, z_max={z_max}, negative={negative}')
...
def main():
    ...
    config = load_config(
        '/mnt/disk1/mapvln/pocket2/pocket2_pipeline/config_grid_map.json')
    ...
    pcd = pass_through_filter(
        pcd, config['thre_z_min'], config['thre_z_max'], bool(
            config['flag_pass_through']))
    pcd = radius_outlier_filter(
        pcd,
        config['thre_radius'],
        config['thres_point_count'])
    grid, origin, width, height = pointcloud_to_gridmap(
        pcd, config['map_resolution'])
    ...
    save_yaml(
        ...
        origin,
        config['map_occupied_thresh'],
        config['map_free_thresh'])
