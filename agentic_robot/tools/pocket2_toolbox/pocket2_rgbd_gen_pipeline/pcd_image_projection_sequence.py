...
class SimplePCDProjection:
    ...
    def load_pcd_with_intensity(self, pcd_file):
        ...
        points, intensities = self.adaptive_downsample(
            points, intensities, pcd_file)
    ...
    def voxel_downsample(self, points, intensities, voxel_size=0.002):
        ...
        self.log(
            f"  ✓ Voxel downsampling complete: {len(points)} → {len(down_points)} points ({ratio:.2%} remaining)")
    ...
    def process_bk(self):
        ...
        try:
            ...
            dilate_view_path = os.path.join(
                DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}_dilate.png")
            depth_output_path = os.path.join(
                DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}.png")
            visualization_output_path = os.path.join(
                DEFAULT_OUTPUT_DIR, f"{cam_id}_{frame_id}_overlay.png")
            self.generate_occ_depth(
                points_image,
                points_camera,
                img_width,
                img_height,
            ...
            # 叠加
            self.log("  生成叠加图像...")
            result = self.overlay_points(
                image, pixel_coords, depths, intensities)
    ...
    def process(self):
        ...
        try:
            ...
            for frame_id in range(0, len(os.listdir(img_dir))):
                ...
                dilate_view_path = os.path.join(
                    self.depth_vis_dir, f"{frame_id:06d}_dilate.png")
                depth_output_path = os.path.join(
                    self.depth_dir, f"{frame_id:06d}.png")
                visualization_output_path = os.path.join(
                    self.depth_vis_dir, f"{frame_id:06d}_overlay.png")
                ...
                self.log(
                    f"每个帧处理时间: {(end_time - start_time) / len(os.listdir(img_dir)):.4f} 秒")
