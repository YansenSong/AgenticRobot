...
class CameraPoseInterpolationUI:
    ...
    def create_widgets(self):
        ...
        self.process_btn = ttk.Button(
            btn_frame, text="开始插值", command=self.start_process)
    ...
    def load_camera_extrinsics(self, path):
        ...
        try:
            ...
            til_pattern = r'Til:\s*\[([^\]]+)\]'
            til_match = re.search(
                til_pattern, content, re.MULTILINE | re.DOTALL)
            if til_match:
                til_values = re.findall(
                    r'[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?',
                    til_match.group(1))
                if len(til_values) >= 16:
                    self.Til = np.array([float(v)
                                        for v in til_values[:16]]).reshape(4, 4)
                    self.log("  ✓ 加载 Til 矩阵")
            else:
                self.Til = None
                self.log("  ⚠ 未找到 Til 矩阵")
            ...
            for cam_id, cam_str in cam_matches:
                ...
                print(
                    f"{self.cameras_intrinsics[cam_id]}:",
                    self.cameras_intrinsics[cam_id])
        ...
        try:
            ...
            for cam_id, Tcl in self.cameras_extrinsics.items():
                ...
                output_file = os.path.join(
                    self.output_dir, f"{cam_id}_pos.txt")
                ...
                with open(output_file, 'w') as f:
                    ...
                    for trigger_id, timestamp in self.trigger_times:
                        ...
                        timestamp, T_w_imu = self.no_interpolate_pose(
                            trigger_id)
                        ...
                        f.write(
                            f"{translation[0]:.10f} {translation[1]:.10f} {translation[2]:.10f} ")
