...
def plot_series(series_list, one_based: bool, out: Path | None, show: bool):
    ...
    if all_vals:
        ...
        ax.axhline(
            ...
            lw=0.8,
            alpha=0.4,
            label=f'overall avg={overall:.3f} ms')
        ax.set_title(' | '.join(
            [f'{n}: {v:.3f} ms' for n, v in avgs]) + f' | overall: {overall:.3f} ms', fontsize=9)
...
def main():
    ap = argparse.ArgumentParser(
        description='Plot LIO times (s->ms) with sample index as X')
    ap.add_argument(
        'files',
        nargs='*',
        help='输入文件，默认: lio_time0.txt lio_time1.txt')
