...
# stop offline mapping (foreground)
Ctrl+C
# or auto-find the exact executable name and stop it
pgrep -x fastlivo_mapping | xargs -r kill
```

```bash
# start online relocalization
export CUDA_VISIBLE_DEVICES=6 
ros2 launch fast_livo online_reloc_{g1/hexfellow}.launch.py
...
# stop online relocalization / localization (foreground)
Ctrl+C
# or auto-find the exact executable names and stop them
pgrep -x online_relo | xargs -r kill
pgrep -x fastlivo_mapping | xargs -r kill
```

### Stop / kill commands

#### 1. Stop foreground launch directly

If mapping or localization is running in the current terminal, press:

```bash
Ctrl+C
```

#### 2. Check related ROS 2 nodes first

```bash
ros2 node list | grep -E "laserMapping|online_relo|republish|rviz2"
```

#### 3. Check related executables first

```bash
pgrep -a -x fastlivo_mapping
pgrep -a -x online_relo
pgrep -a -x republish
pgrep -a -x rviz2
```

#### 4. Auto-kill by exact executable name

`pgrep -x` only matches the exact process name, so it avoids `pkill -f` style substring accidental kills.

Offline mapping / online livo main executable:

```bash
pgrep -x fastlivo_mapping | xargs -r kill
```

Online relocalization main executable:

```bash
pgrep -x online_relo | xargs -r kill
```

Image republish process if needed:

```bash
pgrep -x republish | xargs -r kill
```

RViz if needed:

```bash
pgrep -x rviz2 | xargs -r kill
```

#### 5. Force kill if normal exit fails

```bash
pgrep -x fastlivo_mapping | xargs -r kill -9
pgrep -x online_relo | xargs -r kill -9
pgrep -x republish | xargs -r kill -9
pgrep -x rviz2 | xargs -r kill -9
```
