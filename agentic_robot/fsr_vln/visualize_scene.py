...
from ovo_slam.run_ovo_mapping import load_representation
...
def main(args):
    ...
    path = Path(args.working_dir) / "data/working/configs/" /\
        dataset_name / args.dataset_info_file
    data_path = Path(args.working_dir) / "data/input/Datasets/"
    with open(path, 'r') as f:
        dataset_info = yaml.full_load(f)

    classes = dataset_info["class_names_reduced"] if dataset_info.get(
        "map_to_reduced", None) else dataset_info["class_names"]
    ...
    if args.visualize_obj or args.visualize_interactive_query or args.visualize_gt_vs_pre:
        ...
        while True:
            if args.visualize_obj:
                visualize_3d_points_obj_id_and_obb(
                    pcd_pred, obj_ids, pcd_colors)
            if args.visualize_interactive_query:
                vis = Visualizer(
                    semantic_module,
                    scene_name=config["data"]["scene_name"],
                    save_path=run_path.parent)
                vis.visualize_and_query(
                    pcd_pred, params["obj_ids"].squeeze().numpy(), pcd_colors)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--working_dir',
        default="${HOLOAGENT_DATA_ROOT}/VLN/OVO",
        type=str)
