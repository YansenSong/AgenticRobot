...
class Visualizer:
    ...
    def _take_snapshot(self, vis):
        vis_utils.take_snapshot(vis,
                                str(self.save_path),
                                self._get_snapshot_name(),
    ...
    def _query_locally(self):
        with self.heatmap_lock:
            self.last_query_map = self.semantic_module.query(
                [self.last_query] + self.query_context).cpu().numpy()
    ...
    def visualize_and_query(self, points, obj_ids, pcd_colors):
        ...
        self.cloud = o3d.t.geometry.PointCloud(
            o3c.Tensor(np.asarray(points), o3c.float32))
    ...
    def create_widgets_window(self):
        ...
        cb_ceilling.set_on_checked(
            self._on_cb_ceilling)  # set the callback function
        ...
        while True:
            ...
            if self.stream_event.wait():
                with self.stream_lock:
                    ...
                    points, colors, c2w = points.astype(
                        np.float32), colors.astype(
                        np.float32), c2w.astype(
                        np.float32)
                    vis_utils.get_camera_frame(cam_lineset,
                                               [cam_intrinsic["width"],
                                                cam_intrinsic["height"]],
                    ...
                    if self.mask_ceiling:
                        ...
                        points, obj_ids, colors = points_w[mask,
                                                           :], obj_ids[mask], colors[mask, :]

                    self.cloud = o3d.t.geometry.PointCloud(
                        o3c.Tensor(points, o3c.float32))
                    self.obj_colors = vis_utils.get_pcd_colors(
                        obj_ids, self.obj_cmap)

                    self.pcd_colors = colors / 255.
                    self.obj_masks, ids = vis_utils.get_obj_ids_and_masks(
                        obj_ids)
                    self.n_objs = len(ids)
                    ...
                    self.main_vis.add_geometry(
                        f"cam_{step}", cam_lineset, mat_cam)
                    if step == 0:
                    ...
                    o3d.visualization.gui.Application.instance.post_to_main_thread(
                        self.main_vis, self.take_snapshot)
...
def visualize_gt_vs_pred(points, gt, pred, labels, labels_idx):
    ...
    o3d.visualization.draw_geometries_with_key_callbacks(
        scene_elements, key_to_callback)
    return


def idxToRGB(seg_image, rgb_image=None, alpha=0.6, max_idx=40):
    """
    Maps each pixel in an image with idx values to RGB.
    ...
    for i in range(idxs_image.max() + 1):
        ...
        if mask.sum() > 0:
            rgb_image[mask] = cmap[i %
                                   len(cmap)] * alpha + (1 - alpha) * rgb_image[mask]
...
def visualize_3d_points_obj_id_and_obb(points, obj_ids, colors):
    ...
    if obj_ids is not None:
        ...
        for i in range(max_level):
            ...
            print("Level", i, "has", len(obb_lvl_list), "objects")
            obb_list.append(obb_lvl_list)
        print("Done loading point cloud and bounding boxes")
    ...
    def next_obj(vis):
        ...
        if BBOX_IDX < len(obb_list[LVL]):
            vis.add_geometry(
                obb_list[LVL][BBOX_IDX][1],
                reset_bounding_box=False)
        if BBOX_IDX > 0:
            try:
                vis.remove_geometry(
                    obb_list[LVL][BBOX_IDX - 1][1], reset_bounding_box=False)
            except BaseException:
                vis.clear_geometries()
        vis.update_renderer()
        return True
    ...
    def prev_obj(vis):
        ...
        if BBOX_IDX < len(obb_list[LVL]):
            vis.add_geometry(obb_list[LVL][BBOX_IDX - 1]
                             [1], reset_bounding_box=False)
        if BBOX_IDX > 0:
            try:
                vis.remove_geometry(
                    obb_list[LVL][BBOX_IDX][1],
    ...
    o3d.io.write_point_cloud("point_cloud_with_colors.ply", point_cloud)
    return
