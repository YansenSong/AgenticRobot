...
class Instance3D:
    ...
    def add_keyframes(self, kf_id: int) -> None:
        """
        If frame  no already in list, add to list of keyframes where the object
        has been observed.
    ...
    def add_top_kf(self, kf_id: int, area: int) -> None:
        """
        If self.n_top_kf <=0, the self.to_update is set to True but self.top_kf
        remains empty.

        Otherwise, if the KF is already on the list of best, update area value. Else, if the area is one of the N biggest, add to list of top keyframes, in both cases self.to_update is set to True. Args:
            - keyframe_id (int): id of keyframe where the object has been observed.
            - area (int): area of the segmentation map on current keyframe
        Return:
            - True if current KeyFrame is in the top_k view, False otherwise
    ...
    def idx_in_top_kf(self, kf_id: int) -> int:
        """
        If kf_id is in self.top_kf, returns the index.

        Otherwise return -1. Args:
    ...
    def is_top_kf(self, kf_id: int) -> bool:
        """
        If kf_id is in self.top_kf, returns True.
    ...
    def update_clip(
            self, keyframes_clips: Dict[int, Dict[int, torch.Tensor]]) -> None:
        """
        If self.to_update is True, compute CLIP vector minimizing L1 norm of
        associated clip vectors from keyframes where the object was observed.

        Minimizing the L1 norm is equivalent to compute the median of the vector's norm. Args:
        ...
        if self.to_update:
            ...
            l1_distances = torch.abs(
                clips - clips.permute(1, 0, 2)).sum((1, 2))
    ...
    def restore(self, obj_dict: Dict[str, Any], debug_info: bool) -> None:
        ...
        self.clip_feature_kf = obj_dict.get(
            f"ins3d_{self.id}_clip_feature_kf", None)
        if debug_info:
            ...
            if obj_dict.get(f"ins3d_{self.id}_top_kfs", None) is not None:
                self.top_kf = [
                    (area, kf_id) for area, kf_id in obj_dict[f"ins3d_{self.id}_top_kfs"]]
    ...
    def old_restore(self, obj_dict: Dict[str, Any], debug_info: bool) -> None:
        ...
        Args:
            ...
            debug_info (bool): If True, expected additional debug information in the dictionary.
        """
        self.clip_feature = torch.tensor(
            obj_dict[f"default_{self.id}_clip_feature"])
        self.clip_feature_kf = obj_dict.get(
            f"default_{self.id}_clip_feature_kf", None)
        if debug_info:
            ...
            if obj_dict.get(f"default_{self.id}_top_kfs", None) is not None:
                self.top_kf = [
                    (area, kf_id) for area, kf_id in obj_dict[f"default_{self.id}_top_kfs"]]

    def purge_points_ids(self, purge_ids: List[int]) -> None:
        """
        TODO: We need to define an heurisitc to purge ids that were optimized
        and do not fall inside the 3D instance.

        -> As long as we use map's point_ins_ids to classify, and not the instance saved points_ids, it doesn't matter if we don't prune.
        """
