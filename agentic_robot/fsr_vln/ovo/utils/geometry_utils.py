...
def match_3d_points(cluster_0: torch.Tensor,
                    cluster_1: torch.Tensor,
                    dist_th: float) -> Tuple[torch.Tensor,
                                             torch.Tensor]:
    """
    Match each point in cluster_0 to its closest point in cluster_1.

    Only matches if dist < dist_th Args:
    ...
    # compute distance
    dist = torch.norm(
        cluster_0.unsqueeze(1).repeat(
            1,
            n,
            1).cpu() -
        cluster_1.cpu(),
        dim=-
        1)  # norm(mxnx3 - nx3) -> mxn

    # filter points further away than th_dist to its closest match
    # mxn -> m, from which i<m have a match
    mask = ((dist < dist_th).sum(-1) > 0)
...
def project_3d_points(
        ...
        w2c: torch.Tensor = None) -> torch.Tensor:
    """
    Project 3D points to the 2D camera frame.

    Move points to camera reference if world to camera (w2c) transform is passed, else assume points are already in camera reference. Args:
...
def match_3d_points_to_2d_pixels(depth: torch.Tensor,
                                 w2c: torch.Tensor,
                                 points_3d: torch.Tensor,
                                 intrinsics: torch.Tensor,
                                 th_dist: float) -> tuple[torch.Tensor,
                                                          torch.Tensor]:
    """
    Match 3D points to 2D camera coordinates in pixels.

    First move gaussian to local reference, filter out non-forward gaussians, project gaussians to 2d plane, match gaussians with projected 2d coordinates if depth distance < th_dist. Args:
    ...
    if points_3d.shape[-1] == 3:
        points_3d = torch.hstack(
            [points_3d, torch.ones((n_points, 1), device=device)])
    ...
    in_plane_mask = torch.logical_and(points_2d[:, 0] < w, points_2d[:, 1] < h) * torch.logical_and(
        points_2d[:, 0] >= 0, points_2d[:, 1] >= 0)  # Points inside frustum already filtered
    # Points inside frustum already filtered
    in_points_2d = points_2d[in_plane_mask]

    # Compute depth of 3d points for points projected inside camera frame
    # depth as z coordinate # Points inside frustum already filtered
    forward_points_depth = forward_points_3d[in_plane_mask, 2]

    # Mask 3d points with distance between their depth and gt depth at the
    # projected point bigger than a threshold
    dist_mask = (forward_points_depth -
                 depth[in_points_2d[:, 1], in_points_2d[:, 0]]).abs() < th_dist
    # We could skip this step because invalid depth values shouldbe filterd by
    # the distance threshold.
    dist_mask[depth[in_points_2d[:, 1], in_points_2d[:, 0]] == 0] = False
    # For 3d points inside the distance threshold, asing the projected 2d
    # point as a match
...
def depth_filter_lidar(depth: torch.Tensor,
                       k_size: int = 7,
                       sigma: float = 2.5,
                       th_ratio: float = 0.15) -> torch.Tensor:
    """
    更适合 LiDAR 稀疏投影 depth 的滤波方法：
    ...
    # 1) 带 mask 的高斯模糊：避免无效点被卷进去
    depth_blur = gaussian_blur(depth[None], kernel_size=k_size, sigma=sigma)
    mask_blur = gaussian_blur(mask[None], kernel_size=k_size, sigma=sigma)

    # 避免除零
    low_freq = (depth_blur / (mask_blur + 1e-5))[0]

    # 2) 局部深度变化
    high_freq = (depth - low_freq).abs()
    ...
    filtered = torch.where(
        high_freq > th, torch.tensor(-1.0, device=depth.device), depth)

    return filtered


def compute_camera_frustum_corners(
        depth_map: torch.Tensor,
        pose: torch.Tensor,
        intrinsics: torch.Tensor) -> torch.Tensor:
    """
    Code from Gaussian-SLAM mapper_utils Computes the 3D coordinates of the
    camera frustum corners based on the depth map, pose, and intrinsics.
    ...
    z = corners[:, 2]
    corners_3d = torch.vstack(
        (x, y, z, torch.ones(x.shape[0], device=depth_map.device))).T
    corners_3d = torch.einsum("ij,mj->mi", pose, corners_3d)
    return corners_3d[:, :3]


def compute_camera_frustum_corners_cpu(
...
def compute_camera_frustum_planes(frustum_corners: np.ndarray) -> torch.Tensor:
    ...
    frustum_corners = frustum_corners.float()
    planes = torch.stack(
        [
            torch.linalg.cross(
    ...
    D = torch.stack([-torch.dot(plane, frustum_corners[i])
                    for i, plane in enumerate(planes)])
    return torch.cat([planes, D[:, None]], dim=1).float()


def compute_frustum_aabb(
        frustum_corners: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Code from Gaussian-SLAM mapper_utils Computes a mask indicating which
    points lie inside a given axis-aligned bounding box (AABB).
    ...
    """
    return torch.min(
        frustum_corners, axis=0).values, torch.max(
        frustum_corners, axis=0).values


def points_inside_aabb_mask(
        points: np.ndarray,
        min_corner: np.ndarray,
        max_corner: np.ndarray) -> np.ndarray:
    """
    Code from Gaussian-SLAM mapper_utils Computes a mask indicating which
    points lie inside the camera frustum.
...
def points_inside_frustum_mask(
        points: torch.Tensor,
...
def compute_frustum_point_ids(
        pts: torch.Tensor,
        frustum_corners: torch.Tensor,
        device: str = "cuda") -> torch.Tensor:
    """
    Code from Gaussian-SLAM mapper_utils Identifies points within the camera
    frustum, optimizing for computation on a specified device.
    ...
    inside_frustum_mask = points_inside_frustum_mask(
        pts[inside_aabb_mask], frustum_planes)
