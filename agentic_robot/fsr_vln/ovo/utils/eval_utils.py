...
# seaborn is changing matplotlib configuration to automatically open
# graphs without calling plt.imshow()
...
def match_labels_to_vtx(points_3d_labels: torch.Tensor,
                        points_3d: torch.Tensor,
                        mesh_vtx: torch.Tensor,
                        filter_unasigned: bool = True,
                        tree: str = "kd") -> Tuple[torch.Tensor,
                                                   torch.Tensor,
                                                   torch.Tensor]:
    ...
    if filter_unasigned:
        ...
        print(
            f"Assigned points {assigned_mask.sum()}, {assigned_mask.float().mean()*100:.1f}")
    ...
    # It may happen that not all instances matched to the predicted point
    # cloud, are matched with the GT mesh. Therefore let's use ids rather than
    # idxs
    ...
    mesh_instances_masks = torch.unsqueeze(
        mesh_labels, dim=0).expand(
        n_instances, -1) == instance_idxs
...
def plot_confmat(
    ...
    sn.heatmap(
        ...
        norm=LogNorm())
    sn.set(font_scale=3)
    # plt.tight_layout() # Removed to avoid RuntimeError with new matplotlib
    # layout engines
    if save:
...
def evaluate_scan(pr_file: str,
    ...
    if map_gt_ids is not None:
        assert isinstance(
            map_gt_ids, dict), "map_gt_ids must be either None or a dictionary which keys map gt label idxs to new idxs"
    ...
    if not pr_ids.shape == gt_ids.shape:
        print(
            f'number of predicted values does not match number of vertices. pred: {pr_ids.shape}; gt: {gt_ids.shape};{pr_file}')
    """
...
def eval_semantics(output_path: str,
                   gt_path: str,
                   scenes: List[str],
    ...
    labels = dataset_info["class_names"] if dataset_info.get(
        "map_to_reduced", None) is None else dataset_info["class_names_reduced"]
    ignore = dataset_info.get("ignore", []).copy()

    if ignore_background:
        if map_to_reduced:
            assert dataset_info.get(
                "background_reduced_ids", None), "To ignore background a list of idxs corresponding to background ids id required!"
    ...
    confusion = np.zeros(
        [len(scenes), num_classes, num_classes], dtype=np.ulonglong)
    ...
    for i in range(len(scenes)):
        iou_values, iou_valid_mask, weights_values, acc_values, acc_valid_mask = iou_acc_from_confmat(
            confusion[i], num_classes, ignore, mask_nan, False, labels)
        print(f"Scene: {scenes[i]}")
        print(
            f'mIoU: \t {np.mean(iou_values[iou_valid_mask]):.2%}; mAcc: \t {np.mean(acc_values[acc_valid_mask]):.2%}\n ')
        ...
        with open(output_path / f"{scenes[i]}_statistics.txt", "w") as f:
            f.write(
                f"mIoU: \t {np.mean(iou_values[iou_valid_mask]):.2%}; mAcc: \t {np.mean(acc_values[acc_valid_mask]):.2%}\n ")
            f.write(f"f-mIoU: \t {np.sum(iou_values[iou_valid_mask]*weights_values[iou_valid_mask])/weights_values[iou_valid_mask].sum():.2%}; f-mAcc: \t {np.sum(acc_values[acc_valid_mask]*weights_values[acc_valid_mask])/weights_values[acc_valid_mask].sum():.2%}\n")

    confusion = confusion.sum(0)  # agregate all scenes statistics
    iou_values, iou_valid_mask, weights_values, acc_values, acc_valid_mask = iou_acc_from_confmat(
        confusion, num_classes, ignore, mask_nan, verbose, labels)
    ...
    if verbose:
        ...
        if iou_values.shape[0] == 51:
            for i, split in enumerate(['head', 'comm', 'tail']):
                ...
                print(
                    f'{split}: \t {np.mean(iou_values[idx_i:idx_e][iou_valid_mask[idx_i:idx_e]]):.2%}')
                print(
                    f'{split}: \t {np.mean(acc_values[idx_i:idx_e][acc_valid_mask[idx_i:idx_e]]):.2%}')
                print('---')
        if isinstance(output_path, str):
            output_path = Path(output_path)
        for i in range(len(scenes)):
            with open(output_path / f"{scenes[i]}_statistics_detail.txt", "w") as f:
                f.write(f"label, acc, iou, \n")
                count = 0
                for i in range(len(labels)):
                    if i not in ignore:
                        f.write(
                            f"{labels[i]}, {acc_values[count]}, {iou_values[count]}, \n")
                        count += 1
...
def eval_scannetpp_semantic(cfg: Dict[str, Any], top_k: List[int] = [
                            1], verbose: bool = True):
    # Import ScanNet++ path
    sys.path.append(
        "/home/tberriel/Workspaces/semsplat_ws/sem3d/ovoslam/submodules/scannetpp")
