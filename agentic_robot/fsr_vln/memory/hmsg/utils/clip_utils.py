...
def _get_model_device_dtype(clip_model):
...
def get_img_feats(img, preprocess, clip_model):
    ...
    model_device, model_dtype = _get_model_device_dtype(clip_model)
    img_in = img_in.to(device=model_device, dtype=model_dtype)
    with torch.no_grad():
...
def get_img_feats_batch(imgs, preprocess, clip_model):
    ...
    model_device, model_dtype = _get_model_device_dtype(clip_model)
    imgs_in = imgs_in.to(device=model_device, dtype=model_dtype)
    with torch.no_grad():
        img_feats = clip_model.encode_image(imgs_in).float()
    img_feats = torch.nn.functional.normalize(img_feats, dim=-1)
    img_feats = np.float32(img_feats.cpu())
    return img_feats
...
def get_imgs_feats_batch(
    ...
    for img_id, img in enumerate(raw_imgs):
        ...
        if len(img_batch) == batch_size or img_id == len(raw_imgs) - 1:
            ...
            img_batch = img_batch.to(device=model_device, dtype=model_dtype)
            with torch.no_grad():
                batch_feats = clip_model.encode_image(img_batch).float()
...
def get_text_feats(in_text, clip_model, clip_feat_dim, batch_size=64):
    """
    Get the text features from the CLIP model :param in_text (list): the text
    to get the features from :param clip_model (CLIP): the CLIP model.
    ...
    model_device, _ = _get_model_device_dtype(clip_model)
    tokenizer = getattr(clip_model, "tokenizer", None)
    if callable(tokenizer):
        text_tokens = tokenizer(in_text)
    else:
        text_tokens = open_clip.tokenize(in_text)

    if isinstance(text_tokens, dict):
        if "input_ids" not in text_tokens:
            raise ValueError("Tokenizer output dict must contain 'input_ids'.")
