...
def generate_long_queries(tree: nx.DiGraph) -> Tuple[List[str], List[Tuple]]:
    """
    Generate long hierarchical queries from the given tree.

    The queries are in the form of <object X in region Y on floor Z>.
...
def generate_gt_object_nodes(
    ...
    """
    The GT object nodes ids.

    Given a hierarchical query "X object in region Y on floor Z", there might be multiple correct target objects. For example,
...
def main(params):
    ...
    scene_dirs = [
        Path(params.paths.data) / "hm3dsem_walks" /
        params.main.split / scene_name
