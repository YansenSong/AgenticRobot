#!/usr/bin/env bash
#
# Build the HexFellow robot-specific ROS 2 workspace.
#
# Usage:
#   bash robots/hexfellow/build.sh
#   bash robots/hexfellow/build.sh -p <pkg>
#   bash robots/hexfellow/build.sh -p "<pkg_a> <pkg_b>"
#   bash robots/hexfellow/build.sh -j 2
#   bash robots/hexfellow/build.sh --dry-run

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
ROBOT_NAME="hexfellow"
